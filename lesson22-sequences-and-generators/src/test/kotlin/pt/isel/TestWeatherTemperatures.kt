...
class TestWeatherTemperatures {
    ...
    @Test
    fun testRainyDaysDescriptionsEager() {
        var descriptions = weatherData
            .filter {
                it.weatherDesc.lowercase().contains("rain")
            }.map {
                it.weatherDesc
        ...
        val order = listOf("1 - ", "2 - ", "3 - ", "4 - ", "5 - ", "6 - ", "7 - ", "8 - ")
        val interleavedSeq = order.interleaveEager(descriptions)

        interleavedSeq.forEachIndexed {
            idx, elem -> if(idx % 2 == 1) println(elem) else print(elem)
        }
    }

    @Test
    fun testRainyDaysDescriptionsLazy() {
        var descriptions = weatherData
            .filter {
                it.weatherDesc.lowercase().contains("rain")
            }.map {
        ...
        val order = sequenceOf("1 - ", "2 - ", "3 - ", "4 - ", "5 - ", "6 - ", "7 - ", "8 - ")
        val interleavedSeq = order
            .asSequence()
            .interleaveLazy(descriptions)
